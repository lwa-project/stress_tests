...
Usage:
  processDrifts.py HDF5_file [HDF5_file [...]]
...
def usage(exitCode=None):
    print """processDrifts.py - Given one or more HDF5 files associated with observations 
generated by generateRun.py, determine the current pointing offset and estimate
the SEFD and FWHM.

Usage: processDrifts.py HDF5 [HDF5 [HDF5]]
...
def parseOptions(args):
    ...
    config['args'] = args
    if len(config['args']) not in (1, 2, 3):
        raise RuntimeError("Must specify between one and three HDF5 files for analysis")
