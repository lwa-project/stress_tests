...
Usage:
  processWeave.py HDF5_file [HDF5_file [...]]
...
def usage(exitCode=None):
    print """processWeave.py - Given a HDF5 files associated with an observation
generated by makeWeave.py, determine the current pointing offset and estimate
the FWHM.

Usage: processWeave.py HDF5
...
def parseOptions(args):
    ...
    if len(config['args']) == 0:
        raise RuntimeError("Must specify at least one HDF5 file for analysis")
    ...
    for filename in filenames:
        ...
        for s in stp:
            ...
            try:
                ...
                decCnt[s[2]] = 1
        raCtr = max(raCnt.iteritems(), key=operator.itemgetter(1))[0]
        decCtr = max(decCnt.iteritems(), key=operator.itemgetter(1))[0]
